"""
health_metrics.py — Pure calculation functions (BMI, water intake, TDEE).

Kept separate from I/O so each formula can be unit-tested in
isolation (see tests/test_health_metrics.py) without needing to
simulate CLI input or touch the ML model.
"""
...
def calculate_tdee(age: int, weight_kg: float, height_m: float, goal: str, gender: str):
    """
    Mifflin-St Jeor equation for BMR, scaled to TDEE at a
    moderate activity level (x1.55), then adjusted by goal.

    Returns (tdee, target_calories, target_label).
    """
